...
class LammpsSysDb:
    ...
    def __init__(self, host, port=None, name=None, username=None,
                 password=None, uri_mode=False, **kwargs):
        self.host = host
        self.db_name = name
        self.user = username
    ...
    def build_indexes(self, indexes=None, background=True):
        self.force_fields.create_index([("smiles", ASCENDING),
                                        ("method", ASCENDING),
                                        ("doi", ASCENDING)], unique=True,
                                       background=background)
        # self.force_fields.create_index("formula_alphabetical", unique=False,
        #                                background=background)
        # # from pmg.molecule.composition.chemical_system
        # self.force_fields.create_index("chemsys", unique=False,
        #                                background=background)
        self.systems.create_index([('species_smiles', ASCENDING),
                                   ('force_field_types', ASCENDING)],
    ...
    def insert_force_field(self, parameter_dict, method, doi=None,
                           update_duplicates=False, **kwargs):
        ff_doc = iluu.process_ff_doc(parameter_dict, method, doi, **kwargs)
        mol = parameter_dict["Molecule"]
        # Check if smiles is already in db
        result = self.force_fields.find_one({"smiles": ff_doc["smiles"],
                                             "method": method,
                                             "doi": doi})
        if result:
            logger.info(f"Parameters for {ff_doc['smiles']} using {method} "
        ...
        if result is None:
            ...
            result_list = list(self
                               .force_fields
                               .find({"formula_alphabetical":
                                         ff_doc["formula_alphabetical"],
                                      "method": method,
                                      "doi": doi}))
            if result_list:
                logger.info(f"Parameters for {ff_doc['smiles']} using "
        ...
        if result and update_duplicates:
            logger.info(f"Updating duplicate parameters for "
        ...
        if result is None or update_duplicates:
            ...
            self.force_fields.update_one({"smiles": ff_doc["smiles"],
                                          "method": method,
                                          "doi": doi},
                                         {"$set": ff_doc}, upsert=True)
            return ff_doc["smiles"]
        else:
            logger.info(f"Skipping duplicate parameters for "
                        f"{ff_doc['smiles']} using {method} according to "
                        f"{doi}")
            return ff_doc["smiles"], method, doi

    def retrieve_force_field(self, smiles, method, doi=None):
        return self.force_fields.find_one({"smiles": smiles,
                                           "method": method,
                                           "doi": doi})

    def delete_force_field(self, smiles, method, doi=None):
        return self.force_fields.delete_one({"smiles": smiles,
                                             "method": method,
                                             "doi": doi})
...
if __name__ == "__main__":
    ...
    test_connect.insert_force_field(wat_param, wat_param["method"], doi=wat_param["doi"])

    na_mol = Molecule.from_file(os.path.join(file_dir, "Na.pdb"))
    na_mol.set_charge_and_spin(1,1)
    na_param = {'Molecule': na_mol,
                     'Labels': ['na'],
