...
class GoutTypeError(Exception):
    def __init__(self, msg=None):
        if msg is None:
            msg = 'no Gaussian output provided; to skip optimization, you ' \
...
def _process_mol_check(working_dir, process_mol_func=True,
                       mol_operation_type=None, mol=None,
                       dir_head=None, mol_name=None,
                       db=None, dir_structure=None):
    if process_mol_func:
    ...
    return mol_operation_type, mol, label, opt_job_name, \
           freq_job_name, working_dir
...
def common_fw(mol_operation_type,
    ...
    mol_operation_type, mol, label, opt_job_name, freq_job_name, working_dir = \
        _process_mol_check(working_dir, process_mol_func,
                           mol_operation_type, mol,
                           dir_head, mol_name, db,
    ...
    if not skips:
        ...
        if skips[0].lower() == 'opt':
            ...
            if mol_operation_type not in ['get_from_gout', 'get_from_gout_file',
                                          'get_from_run_dict',
                                          'get_from_run_id',
                                          'get_from_run_query']:
                raise GoutTypeError()
            else:
                freq_fw = CalcFromMolFW(mol=mol,
                                        mol_operation_type=mol_operation_type,
                                        db=db,
                                        name=freq_job_name,
                                        working_dir=os.path.join(working_dir,
        ...
        if mol_operation_type not in ['get_from_gout', 'get_from_gout_file',
            ...
            if check_result:
                ...
                not_found_ind = [ind for ind, boolean in enumerate(keys_exist)
                                 if not boolean]
                if not_found_ind:
                    not_found_keys = [check_result[ind] for ind in
                                      not_found_ind]
