...
class ProcessPrmtop(FiretaskBase):
    ...
    optional_params = ["working_dir", "prmtop_path", "prmtop_filename", "prmtop_dir", "unique_molecule_name",
    ...
    def run_task(self, fw_spec):
        ...
        if isinstance(self.get("prmtop_path"), str):
            ...
            raise Exception('Neither name nor path to a prmtop file was provided; '
                            'either provide prmtop file name as "prmtop_filename" or '
                            'path to prmtop as "prmtop_path".')

        if not os.path.exists(prmtop_file_path):
            raise Exception('"prmtop_file_path" is not a valid path; check that "prmtop_path" is '
                            'correct or that "prmtop_filename" exists in "prmtop_dir"')
        ...
        # TODO: decide what unique id for the ff labels. If staying as smiles string, decide how to get smiles.
        ...
        ff_param_dict_system =  add_ff_labels_to_dict(ff_param_dict_general, unique_mol_name)
        ...
        return FWAction(stored_data = {'ff_param_dict_system': ff_param_dict_system,
