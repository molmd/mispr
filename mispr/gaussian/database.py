...
class GaussianCalcDb:
    ...
    def __init__(self, host, port=None, name=None, username=None, password=None,
        ...
        try:
            if uri_mode:
                ...
                self.connection = \
                    MongoClient(self.host, self.port,
                                ssl=kwargs.get('ssl', False),
                                ssl_ca_certs=kwargs.get('ssl_ca_certs'),
                                ssl_certfile=kwargs.get('ssl_certfile'),
                                ssl_keyfile=kwargs.get('ssl_keyfile'),
                                ssl_pem_passphrase=kwargs.get(
                                    'ssl_pem_passphrase'),
                                ssl_cert_reqs=kwargs.get('ssl_cert_reqs',
                                                         ssl.CERT_NONE),
                                username=username,
                                password=password,
    ...
    def insert_molecule(self, mol, update_duplicates=False):
        ...
        if result is None:
            m = MoleculeMatcher()
            result_list = list(
                self.molecules.find({'formula_alphabetical':
                                         mol_dict['formula_alphabetical']}))
            if result_list:
                logger.info(
                    '{} already in {} documents'.format(mol_dict['inchi'],
                                                        len(result_list)))
    ...
    def insert_fg(self, fg_file):
        ...
        list_of_groups = [i for i in list_of_groups if
                          i['name'] not in existing]
    ...
    def insert_derived_mol(self, derived_mol, update_duplicates):
        if isinstance(derived_mol, Molecule):
            ...
            if result:
                logger.info(
                    '{} already in database'.format(derived_mol_dict['inchi']))
            if result and update_duplicates:
                logger.info(
                    'Updating duplicate {}'.format(derived_mol_dict['inchi']))
            if result is None or update_duplicates:
