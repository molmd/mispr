...
class Node:
    def __init__(self, state: str, phase: str, num_electrons: int,
                 mol_operation_type=None, mol=None, skips=None,
                 check_result=None, parent: 'Node' = None, ref_charge=None,
    ...
    def create_fireworks(self, opt_gaussian_inputs, freq_gaussian_inputs,
                         ...
                         working_dir, db, cart_coords, branch_cation_from_anion,
                         **kwargs):
    ...
    def branch(self, branching_states, branching_phases, num_of_electrons,
               branch_cation_from_anion, h_index, vertical):
        ...
        for phase in branching_phases:
            ...
            self.children_nodes.append(Node(
                ...
                branch_cation_from_anion=branch_cation_from_anion, skips=skips))
...
def get_ip_ea(mol_operation_type,
    ...
    if electrode_potentials:
        electrode_potentials = \
            ...
            if isinstance(v, dict) else v for k, v in
             electrode_potentials.items()}

        for k, v in electrode_potentials.items():
            if type(v) != dict or 'potential' and 'ref' not in v:
                raise KeyError('Standard electrode potential dict should '
    ...
    root_node = Node('reference', 'gas' if 'gas' in phases else 'solution',
                     ...
                     skips=ref_skips, check_result=check_result,
                     ref_charge=ref_charge, branch_cation_from_anion=pcet)
    solved_nodes = []
    active_nodes = Queue()
    ...
    while not active_nodes.empty():
        ...
        current_node.create_fireworks(
            ...
            solvent_properties, working_dir, db, cart_coords, pcet, **kwargs)
        ...
        if abs(current_node.added_e) <= num_electrons or \
            ...
            children = current_node.branch(branching_states,
                                           branching_phases,
                                           addition_electrons,
                                           pcet,
                                           h_index,
